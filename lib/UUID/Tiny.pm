...
use constant UUID_TIME   => 1;
use constant UUID_V3     => 3;
use constant UUID_MD5    => 3;
use constant UUID_V4     => 4;
use constant UUID_RANDOM => 4;
use constant UUID_V5     => 5;
use constant UUID_SHA1   => 5;
...
sub _create_v3_uuid
{
    ...
    {
        croak __PACKAGE__ . '::create_uuid(): Name for v3 UUID' .
...
sub _create_v5_uuid
{
    ...
    {
        croak __PACKAGE__ . '::create_uuid(): Name for v5 UUID' .
          ' has to be SCALAR, GLOB or IO object, not ' .
          ref($name) . '!';
    }
    elsif ( defined $name )
